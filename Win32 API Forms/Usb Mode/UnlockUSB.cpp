...
//{	
// system("call UnlockUSB.bat");
// system("echo 1 >Pathe6");
// system("attrib +h +s Pathe6");
         //std::string STRING;
	     //std::ifstream infile;
	    //infile.open ("Pathe6");
 	...
 	while (fread(CreatePassword,recsizelogin,1,loginFile) == 1)              //Read string from a file and store into a variable e with loop
			...
			if (strcmp(password , CreatePassword) == 0)        
				...
				else
				{
				  ...
				  MessageBox(hwnd, "Your Password is not Correct or You Did not Lock Files Previously", "ho!", MB_OK);
