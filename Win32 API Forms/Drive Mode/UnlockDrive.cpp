...
void BrowsePath(HWND hwnd)
{
 ...
 system("call UnlockDrive.bat");
 system("echo 1 >Pathe8");
...
{	
  ...
  CreateWindow(TEXT("STATIC"),TEXT("Path::"),WS_CHILD | WS_VISIBLE |WS_CHILD, 6,30,40,20,hwnd,0,0,0);
         ...
         CreateWindow(TEXT("button"), TEXT("Browse"),
                    ...
                    CreateWindow(TEXT("STATIC"),TEXT("Unlock Password::"),WS_CHILD | WS_VISIBLE |WS_CHILD, 30,120,120,20,hwnd,0,0,0);
...
void Unlock(HWND hwnd)
{
    ...
    GetWindowText(hpassword,password,30);
     ...
     for(int i = 0; (i < 100 && password[i] != '\0'); i++)
         ...
         MessageBox(hwnd, "System Cannot Found Lock Files With Password ", "ho!", MB_OK);
			...
			if (strcmp(password , CreatePassword) == 0)        
				{
                 ...
                 system("echo Drive Files Unlocked at %date% with %time% >>FilesActive");
				 system("echo. >>FilesActive");
				...
				else
				{
				  ...
				  MessageBox(hwnd, "System Cannot Found Lock Files With Folder ", "ho!", MB_OK);
